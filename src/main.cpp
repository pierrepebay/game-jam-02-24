...
int main()
{
  ...
  // ------------------------------------
  Shader ourShader(cubeVertexPath.c_str(), cubeFragmentPath.c_str());
  ...
  unsigned char *data = stbi_load(mogTexturePath.c_str(), &width, &height, &nrChannels, 0);
  if (data)
  {
  ...
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
  // load image, create texture and generate mipmaps
  data = stbi_load(mogTexturePath.c_str(), &width, &height, &nrChannels, 0);
  if (data)
  {
